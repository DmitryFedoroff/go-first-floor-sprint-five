...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	calories := (CaloriesMeanSpeedMultiplier * CaloriesMeanSpeedShift * r.meanSpeed() * r.Weight * r.Duration.Hours() * MinInHours) / MInKm
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	if w.Duration == 0 || w.Height == 0 {
		return 0.0
	}
	speedMsec := w.meanSpeed() * KmHInMsec
	heightMeters := w.Height / CmInM
	return (CaloriesWeightMultiplier*w.Weight + (math.Pow(speedMsec, 2)/heightMeters)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	if s.Duration == 0 || s.LengthPool == 0 {
		return 0.0
	}
